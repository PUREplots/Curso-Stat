...
f <- factor(x, labels = c('Frio', 'Caliente'))
levels(f)
...
Bichos <- data.frame(Tipo = c('A', 'Q', 'A', 'P', 'P'),
                     Num.Bichos = c(3, 10, 2, 12, 1))

View(Bichos)

Bichos %>% group_by(Tipo) %>% summarize(mean(Num.Bichos))

Bichos$Tipo
Bichos$Num.Bichos

# como transformar un vector de 5 factores a uno de 3:
# labels <- etiquetas sobre categorías
# levels <- categorías

f <- factor(clima$Events)
levels(f) <- c("seco", "humedo", "humedo", "mojado", "mojado")
levels(f)

Nuevos.Bichos <- c(4,3,12,12,1)
Bichos %>% mutate(Num.Bichos = Nuevos.Bichos)
